...
void calculateMap()
{
    ...
    const double phiStart = PI / 12 + PI / 2;
    ...
    for (int iGroup = 0; iGroup < sectorTileMap.size(); iGroup++)
    {
        ...
        for (int iRow = 0; iRow < nRows; iRow++)
        {
            ...
            double rTop = getR(globalZ, etaBins[iRow + 1]); // closest tile distance to the beam pipe

            double rBottom = getR(globalZ, etaBins[iRow]);
            ...
            for (int iTile = 0; iTile < nTiles; iTile++)
            {
                for (int iEta = 0; iEta < nPointsPerEta + 1; iEta++)
                {
                    ...
                    for (int iPhi = 0; iPhi < nPointsPerPhi + 1; iPhi++)
                    {
                        ...
                        double theta = 180 - 180 * getTheta(x, y) / PI;
